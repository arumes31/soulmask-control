...
## 2026-05-24 - [Default Admin Credential]
**Vulnerability:** The application used a hardcoded, guessable default password ("admin") when the `ADMIN_PASSWORD` environment variable was not set. This allows attackers to easily gain administrative access to applications deployed without properly configuring this variable.
**Learning:** Hardcoded defaults for sensitive credentials create significant risk for users who don't override them, making the application "insecure by default".
**Prevention:** Never use predictable default values for authentication credentials. Instead, securely generate a strong, random password on startup and print it to the server logs, forcing the administrator to explicitly fetch it or set a custom password.
