...
## 2026-05-17 - [Hardcoded Default Admin Password]
**Vulnerability:** The application used a hardcoded default password (`"admin"`) if the `ADMIN_PASSWORD` environment variable was not set, allowing any unauthorized user to access the control panel on deployments that lacked explicit configuration.
**Learning:** Default fallbacks for sensitive secrets, even if intended for "ease of setup", create critical vulnerabilities in unconfigured environments. Security must be the default, not an opt-in configuration.
**Prevention:** Never use hardcoded fallback passwords. If a required secret is missing, the application should either fail to start securely or generate a strong random secret and log it for the administrator to use.
