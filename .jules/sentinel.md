...
## 2026-05-03 - [Missing Rate Limiting on Login Endpoint]
**Vulnerability:** The `/login` endpoint lacked rate limiting, allowing an attacker to perform unlimited brute force or dictionary attacks against the admin password.
**Learning:** Even with secure password comparison, the absence of rate limiting turns a login form into a high-risk vector. Network-level IP tracking is needed to restrict excessive login attempts.
**Prevention:** Always implement an IP-based or user-based rate limiter on authentication endpoints. Ensure to include memory leak prevention if using an in-memory map to track request attempts.
