...
## 2026-05-21 - [Hardcoded Default Admin Password]
**Vulnerability:** The application used a hardcoded default password (`"admin"`) when the `ADMIN_PASSWORD` environment variable was not set. This meant any instance deployed without explicit configuration would be instantly vulnerable to unauthorized access by anyone trying the default credentials.
**Learning:** Hardcoded default credentials are a critical security risk as they provide a universal backdoor if configuration is missed. Even for self-hosted apps, the "secure by default" principle must apply.
**Prevention:** Never use predictable or hardcoded fallback credentials. If a required secret (like an admin password) is missing at startup, either fail fast and exit, or securely auto-generate a random, high-entropy password and log it to stdout for the administrator to use.
