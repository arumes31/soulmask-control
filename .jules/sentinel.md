...
## 2026-05-12 - [Missing Rate Limiting on Login Endpoint]
**Vulnerability:** The `/login` endpoint did not implement any form of rate limiting. This allowed attackers to perform brute-force attacks against the admin password by sending unlimited requests in a short time.
**Learning:** This architectural gap means that a weak password can be quickly compromised through automation. Any public-facing authentication endpoint needs protective measures to prevent credential stuffing and brute-forcing.
**Prevention:** Always implement IP-based or user-based rate limiting on sensitive endpoints, especially login routes. A strategy like allowing a small number of failed attempts followed by a mandatory timeout period is an effective countermeasure.
